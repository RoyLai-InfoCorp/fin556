...
const App = () => {
  ...
  const handleSell = async (aOrB) => {
    if (aOrB === "A" && !amtA) {
      alert("Invalid TokenA amount");
      return;
    }

    if (aOrB === "B" && !amtB) {
      alert("Invalid TokenB amount");
      return;
    }
    ...
    try {
      if (aOrB === "A") {
        const outputAmt = await sellTokens(
          amtA,
          tokenAddrA,
          tokenAddrB,
          signer,
        ...
        console.log(`Sold ${amtA} of A for ${outputAmt} of B`);
      } else if (aOrB === "B") {
        const outputAmt = await sellTokens(
          amtB,
          tokenAddrB,
        ...
        console.log(`Sold ${amtB} of B for ${outputAmt} of A`);
      } else {
